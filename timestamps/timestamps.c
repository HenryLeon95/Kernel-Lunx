...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Henry Francisco - León Hernández");
...
{
    return 0; //Escribió correctamente el tiempo
}
...
{
    struct proc_dir_entry *entry;   // Crea el archivo
    entry = proc_create("timestamps", 0777, NULL, &my_fops); //Creamos una nueva estructura llamada timestamps, crea el módulo como tal dentro de la carpeta proc
    if (!entry)
    {
        return -1;
    }
...
//  sudo apt-get install linux-headers-$(uname -r) instala los headers, librerías